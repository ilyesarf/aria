...
void initMenuEnigmeButtons(Button *buttons) {
    buttons[0].normalImage = load_image("buttons/quiz.png");
    buttons[0].hoverImage = load_image("buttons/quizhov.png");
    buttons[0].rect = (SDL_Rect){600,200,300,40}; // Quiz
    buttons[0].selected = 0;

    buttons[1].normalImage = load_image("buttons/puzzle.png");
    buttons[1].hoverImage = load_image("buttons/puzzlehov.png");
    buttons[1].rect = (SDL_Rect){1000,200,300,40}; // Puzzle
    buttons[1].selected = 0;

    buttons[2].normalImage = load_image("buttons/butbase.png");
    buttons[2].hoverImage = load_image("buttons/butbase.png");
    buttons[2].rect = (SDL_Rect){400,300,300,40}; // Option 1
    buttons[2].selected = 0;

    buttons[3].normalImage = load_image("buttons/butbase.png");
    buttons[3].hoverImage = load_image("buttons/butbase.png");
    buttons[3].rect = (SDL_Rect){400,400,300,40}; // Option 2
    buttons[3].selected = 0;

    buttons[4].normalImage = load_image("buttons/butbase.png");
    buttons[4].hoverImage = load_image("buttons/butbase.png");
                    ...
                    if (buttons[i].selected) {
                        if (i == 0) {
                            ...
                            if (suspenseMusic == NULL) {
                                suspenseMusic = Mix_LoadMUS("suspense.mp3");
                                if (!suspenseMusic) {
