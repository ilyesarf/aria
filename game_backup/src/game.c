...
#define GROUND_Y (SCREEN_HEIGHT - 50)  // Adjusted to be higher up
...
void init_player(Player* player, const char* sprite_path, int x) {
    ...
    printf("Player initialized at: x=%d, y=%d\n", player->pos.x, player->pos.y);  // Debug info
}
...
void display_player(SDL_Surface* screen, Player* player) {
    // Only try to display if we have a valid image and position
    if (player->img && player->pos.w > 0 && player->pos.h > 0) {
        SDL_BlitSurface(player->img, &player->posSprite, screen, &player->pos);
        draw_player_health_bar(screen, player);
    } else {
        printf("Warning: Invalid player state for display\n");  // Debug info
    }
}
