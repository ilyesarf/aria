...
void initBalls() {
    for (int i = 0; i < MAX_BALLS; i++) {
        balls[i].active = 0;
        balls[i].img = IMG_Load("ball.png");
        if (balls[i].img == NULL) {
            printf("Unable to load ball image: %s\n", SDL_GetError());
            balls[i].img = SDL_CreateRGBSurface(SDL_HWSURFACE, BALL_WIDTH, BALL_HEIGHT, 32, 0, 0, 0, 0);
            SDL_FillRect(balls[i].img, NULL, SDL_MapRGB(balls[i].img->format, 255, 0, 0));
        }
    }
}
...
void createBall(character* player) {
    for (int i = 0; i < MAX_BALLS; i++) {
        if (!balls[i].active) {
            balls[i].pos.x = player->pos.x + player->pos.w/2 - BALL_WIDTH/2;
            balls[i].pos.y = player->pos.y + player->pos.h/2 - BALL_HEIGHT/2;
            balls[i].pos.w = BALL_WIDTH;
            balls[i].pos.h = BALL_HEIGHT;
           
            if (player->posSprite.y == 156) { // facing left
                balls[i].dx = -BALL_SPEED;
...
void createBall1(character* player2) {
    for (int i = 0; i < MAX_BALLS; i++) {
        if (!balls[i].active) {
            balls[i].pos.x = player2->pos.x + player2->pos.w/2 - BALL_WIDTH/2;
            balls[i].pos.y = player2->pos.y + player2->pos.h/2 - BALL_HEIGHT/2;
            balls[i].pos.w = BALL_WIDTH;
            balls[i].pos.h = BALL_HEIGHT;
           
            if (player2->posSprite.y == 156) { // facing left
                balls[i].dx = -BALL_SPEED;
            } else { // facing right or default
                balls[i].dx = BALL_SPEED;
            }
...
void updateBalls() {
    for (int i = 0; i < MAX_BALLS; i++) {
        if (balls[i].active) {
            ...
            if (balls[i].pos.x < 0 || balls[i].pos.x > SCREEN_W ||
                balls[i].pos.y < 0 || balls[i].pos.y > SCREEN_H) {
                balls[i].active = 0;
            }
        }
    }
...
void displayBalls(SDL_Surface* screen) {
    for (int i = 0; i < MAX_BALLS; i++) {
        if (balls[i].active) {
            SDL_BlitSurface(balls[i].img, NULL, screen, &balls[i].pos);
        }
    }
}
...
i->space=1;
break;
case SDLK_h: // Added for ball attack
                    i->j = 1;
                    break;
case SDLK_m:
                    i->dec = 1;
...
case SDLK_UP:
                    i->jump=1;
                    break;
case SDLK_j: // Added for ball attack
                    i->h = 1;
                    break;
...
void move_character(in* input, character* c,character* c2,Uint32 dt) {
    ...
    if ((input->q==1) && (c2->pos.x>=10)&& (c2->spd>=0) ) {
        c2->pos.x -=((1/2)*(c2->acc)*(dt*dt))+(c2->velocity*dt)/2.5;
    }
...
if ((input->right==1) && (c->pos.x<=1280) &&(c->spd>=0)) {
        c->pos.x +=((1/2)*(c->acc)*(dt*dt))+(c->velocity*dt)/2.5;
